...
kdtree_node* KdTree::build_tree(size_t depth, size_t a, size_t b) {
  ...
  if (b - a <= 1) {
    ...
    node->points_in_bound.reserve(__sz);
    if(node->loson) 
...
bool operator<=(const CoordPoint& a, const CoordPoint& b)
{
  if(a.size()!=b.size()) throw std::length_error("CoordPoints must have the same length!");
...
std::vector<CoordPoint> KdTree::list_points_in_box(kdtree_node* h, const CoordPoint& lo, const CoordPoint& hi)
{
  ...
  if(h==nullptr)  return res;
  auto cutdim=h->cutdim;
  if(lo<=h->lobound&&h->upbound<=hi)  return h->points_in_bound;
  if(hi<=h->lobound)  return res;
  if(h->upbound<=hi)  return res;
  if(h->cutval>=lo[cutdim])
  {
    res=list_points_in_box(h->loson, lo, hi);
    if(h->cutval<=hi[cutdim])
    {
      auto tmp=list_points_in_box(h->hison, lo, hi);
      res.insert(res.end(),tmp.begin(),tmp.end());
    }
  }
  else
  {
    res=list_points_in_box(h->hison, lo, hi);
  }
  // std::cout<<"points at this node: ";
  // for(auto e:h->points_in_bound)
  // {
  //   std::cout<<" ( ";
  //   for(auto ee:e)  std::cout<<ee<<' ';
  //   std::cout<<") ";
  // }
  // std::cout<<"lobound: "; for(auto e:h->lobound) std::cout<<e<<' ';
  // std::cout<<"upbound: "; for(auto e:h->upbound) std::cout<<e<<' ';
  // std::cout<<"cutdim: "<<h->cutdim<<" cutval: "<<h->cutval<<'\n';
  return res;
}
